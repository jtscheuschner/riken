...
void MakeTree::main(short run, Bool_t test){
  ...
  else             fstore->Open(Form("data/ridf/ggdaq04/calib2XO%04d.ridf",run-500)); 
...
Double_t MakeTree::QTC_Time(Double_t e_mid, Double_t e_low, UInt_t crystal){
  ...
  /*  if(e_mid < 10e3){
    corr = TMath::Exp(fQTC_slopecorr[crystal][0]*fQTC_slopecorr[crystal][1])+
      ...
      /*
    corr = fQTC_slopecorr[0]+fQTC_slopecorr[1]*e_mid +
      TMath::Exp(fQTC_slopecorr[2]*fQTC_slopecorr[3])+
      TMath::Exp(fQTC_slopecorr[4]*fQTC_slopecorr[5]);*/
    //corr=-TMath::Exp(fQTC_slopecorr[0]*fQTC_slopecorr[1])+fQTC_slopecorr[2];
    //if(e_mid < 12e3){
    /*corr = fQTC_slopecorr[0]+
    fQTC_slopecorr[1]*e_mid+
    fQTC_slopecorr[2]*e_mid*e_mid+
    fQTC_slopecorr[3]*e_mid*e_mid*e_mid+
    fQTC_slopecorr[4]*e_mid*e_mid*e_mid*e_mid+
    fQTC_slopecorr[5]*e_mid*e_mid*e_mid*e_mid*e_mid+
    fQTC_slopecorr[6]*e_mid*e_mid*e_mid*e_mid*e_mid*e_mid+
    fQTC_slopecorr[7]*e_mid*e_mid*e_mid*e_mid*e_mid*e_mid*e_mid;
    *//*
  }else{
    corr = fQTC_slopecorr_low[crystal][0]+
      fQTC_slopecorr_low[crystal][1]*e_low;
      //fQTC_slopecorr_low[crystal][2]*TMath::E()^(fQTC_slopecorr_low[crystal][3]*e_low);
      }*/
  if(e_mid>0){
    corr = (am[crystal][0]+
...
void MakeTree::PID(){
  ...
  ZET[3] = fIC->GetZCoef(0)*TMath::Sqrt((fIC->GetEnergySqSum()+fESqoff)/de_v)*BETA[3]+fIC->GetZCoef(1);
