#include "MakeTree_jojo_diff_MidSlew.h"
#include <iostream>
#include "Riostream.h"
#include <string>
#include <time.h>
...
void MakeTree::main(short run, Bool_t test){
  ...
  else fout = new TFile(Form("data/rootfiles/run%04d.root",run),"RECREATE");
...
void MakeTree::LaBr(){
  ...
  for(short ilabr=0; ilabr<fn_labr; ilabr++){
    ...
    QTCTime[ilabr] = (qtc_dummy[0][fQTC[1][ilabr]] - tref)*ftime - ftof_F8_T;
    ...
    Double_t corr_time = QTC_Time(qtc_dummy[1][ fQTC[1][ilabr] ] - qtc_dummy[0][ fQTC[1][ilabr] ],0,ilabr);//MIDGAIN[ilabr], LOWGAIN[ilabr],ilabr);
    QTCTimeC[ilabr] = QTCTime[ilabr] - corr_time;
  }//for
}
...
void MakeTree::ReadQTC(){
  ...
  Float_t qtcslope[3][8]={1.5194, 1.3509, 1.54495,1.3224,1.51863,1.54124,1.31455,1.43441,
  ...
  Float_t qtcoffset[3][8]={-2221.76,-1986.75,-2289.73,-2053.94,-2250.28,-2214.31,-1911.24,-2082.49,
  ...
  UInt_t hig[8] = {28, 29, 30, 1, 31, 4, 3, 2};
  UInt_t mig[8] = {24, 25, 26, 6, 27, 9, 8, 7};
  UInt_t log[8] = {20, 21, 22,11, 23,14,13,12};//?logic maybe 10,13,12,11??
  Float_t QTCoffset[8] = {-1037., -1040., -1034., -1030., -1042., -1031., -1031., -1031.};
  for(short ii=0; ii<8;ii++){
    fQTCTimeoffset[ii]=QTCoffset[ii];
    fQTC[0][ii] = hig[ii];
    fQTC[1][ii] = mig[ii];
    fQTC[2][ii] = log[ii];
...
void MakeTree::AddTree(UInt_t run){
  ...
  fTree->Branch("QTCTimeC",QTCTimeC,"QTCTimeC[8]/F");        
