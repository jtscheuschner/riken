...
#ifndef ZET_Correction_h
#define ZET_Correction_h
...
public :
  ...
  Bool_t Incoming();
  Bool_t Outgoing();
  Double_t fvar[2][4];//two IC and 4 variables for ZET
  Double_t zet(short ic);
  ...
  TObjArray* fdummy;
  //histograms
  TH2F* fh2_a[kMaxBigRIPSIC][kMaxICChannel];
  TH2F* fh2_x[kMaxBigRIPSIC][kMaxICChannel];
  TH2F* fh2_y[kMaxBigRIPSIC][kMaxICChannel];
  TH2F* fh2_b[kMaxBigRIPSIC][kMaxICChannel];
  TH2F* fh2_z[kMaxBigRIPSIC];
  ...
  TArtIC* fIC;
   ...
   ZET_Correction(TTree *tree=0, Float_t AoQ = 2.64,TString name = "");// = "data/rootfiles/test/run0001_299.800_-159.735.root");
   virtual ~ZET_Correction();
...
#ifdef ZET_Correction_cxx
ZET_Correction::ZET_Correction(TTree *tree,Float_t AoQ, TString name) : fChain(0) 
{
  ...
  else if( name == "data/rootfiles/test/run0010_299.793_-158.910.root")   path = "cutParameters/PID/Sn132";
  ...
  else { path = "cutParameters/PID/Sn132"; std::cout<< " no path found" <<std::endl;}
  f_aoq = AoQ;
}

ZET_Correction::~ZET_Correction()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t ZET_Correction::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ZET_Correction::LoadTree(Long64_t entry)
{
...
void ZET_Correction::Init(TTree *tree)
{
...
void ZET_Correction::Show(Long64_t entry)
{
...
Int_t ZET_Correction::Cut(Long64_t entry)
{
...
#endif // #ifdef ZET_Correction_cxx
