...
// Tue Mar 10 10:46:46 2015 by ROOT version 5.34/25
// from TTree tree/tree
// found on file: data/rootfiles/Sn132_LHe/run0009.root
//////////////////////////////////////////////////////////

#ifndef analyse_DALI_h
#define analyse_DALI_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   const Int_t kMaxDALINaI = 86;
   const Int_t kMaxBigRIPSRIPS = 5;
   const Int_t kMaxBigRIPSTOF = 3;
   const Int_t kMaxBigRIPSBeam = 7;
...
public :
   ...
   UInt_t selected_events;
   //selfdefined functions
   
   /** Gamma's **/
   void ADC();//DALI and LaBr (95+8)
   void QTC();//LaBr (3*8)
   //  void Addback();
  /** Beam **/
   void Tke();//total kinetic energy counter plot some histos
   void Beta();//plots some histos for beta
   void BeamProfile();//gives back histograms about the beam f7-f9
   /** Cuts  **/
   Bool_t ADC_time(short dali);//timing
   Bool_t QTC_time(short crystal, short channel, Double_t LaBr_time);//timing
   Bool_t PID(short run);
   Bool_t trigger();//choose desired trigger, Beam, DALI, DALISum, LaBr
   /** Initializer **/
   void Histo();//Define all the histo variables
   void LoadData(TTree *tree,short run);//Load the tree variables
   void set(short run, Bool_t pieter);//set run-specifiq things like the cuts
   void reset();   //reset the run-specifiq things
   void reset_run();   //reset the event-specifiq things
   void ReadPIDCuts(short run); // Read run by run the PID values mean and sigma to cut on them

   /** funciton **/
   Double_t tri_exp(Double_t En);

   //Global variables
   /** cuts **/
   //PID
   Float_t Sigma;//how many sigma around the peak
   //mean and sigma of the variables AOQ and ZET
   Float_t AOQ_ms[2][4];//0 mean 1 sigma 
   Float_t ZET_ms[2][4];//0 incoming 1-3 outgoing: 1,2,5
   //trigger
   short trigg;
   ...
   analyse_DALI(TTree *tree=0);
   virtual ~analyse_DALI();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(short end, short trig, Float_t _sigma, Bool_t pieter = kFALSE);
...
#ifdef analyse_DALI_cxx
analyse_DALI::analyse_DALI(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("data/rootfiles/new/run0009.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("data/rootfiles/new/run0009.root");
      }
...
void analyse_DALI::LoadData(TTree* tree, short run){
// .lsif parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
     TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(Form("data/rootfiles/new/run%04d.root",run));
      if (!f || !f->IsOpen()) {
	f = new TFile(Form("data/rootfiles/new/run%04d.root",run));
      }
      f->GetObject("tree",tree);
...
Int_t analyse_DALI::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t analyse_DALI::LoadTree(Long64_t entry)
{
...
void analyse_DALI::Init(TTree *tree)
{
...
void analyse_DALI::Show(Long64_t entry)
{
...
Int_t analyse_DALI::Cut(Long64_t entry)
{
