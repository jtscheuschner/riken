...
// Mon Apr 27 15:29:34 2015 by ROOT version 5.34/25
// from TTree tree/tree
// found on file: run0102_301.389_-156.264.root
//////////////////////////////////////////////////////////
...
public :
  ...
  void Fitter(TH2F* h2, Int_t var, Bool_t clear);

  Double_t correction[2][variables];
   ...
   virtual void     Loop(short aoq, short max);
...
#ifdef Imp_PID_cxx
Imp_PID::Imp_PID(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
/*   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("data/rootfiles/test/run0102_301.388_-156.264.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("data/rootfiles/test/run0102_301.389_-156.264.root");
      }
...
Int_t Imp_PID::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Imp_PID::LoadTree(Long64_t entry)
{
...
void Imp_PID::LoadData(TTree* tree, short run){
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
     TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("data/rootfiles/test/run0102_301.510_-156.164.root");
      if (!f || !f->IsOpen()) {
	f = new TFile(Form("data/rootfiles/test/run0102_301.510_-156.164.root",run));
      }
      f->GetObject("tree",tree);
...
void Imp_PID::Show(Long64_t entry)
{
...
Int_t Imp_PID::Cut(Long64_t entry)
{
